...
?>
	<div class="container">
		...
		<?php if ( $text ) : ?>
			<?php echo amb_site_get_the_content( $text ); // phpcs: xss: ok. ?>
		<?php endif; ?>

		<?php if ( $accordion_items ) : ?>
			<div class="accordion" aria-label="<?php esc_attr_e( 'Accordion Content Block', 'advanced-maps-block' ); ?>">
				<?php
				$count = 0;
				while ( have_rows( 'accordion_items' ) ) :
						...
						<div class="accordion-item">
							<div class="accordion-item-header">
								<h3 class="accordion-item-title"><?php echo esc_html( $item_title ); ?>
									<button class="accordion-item-toggle" aria-expanded="false" aria-controls="<?php echo esc_attr( $item_content_id ); ?>">
										<span class="screen-reader-text"><?php echo sprintf( esc_html( 'Toggle %s', 'advanced-maps-block' ), esc_html( $item_title ) ); ?></span>
							...
							<div id="<?php echo esc_attr( $item_content_id ); ?>" class="accordion-item-content" aria-hidden="true">
								<?php echo amb_site_get_the_content( $item_content ); // phpcs: xss: ok. ?>
							</div>
						</div>
				<?php endwhile; ?>
			</div>
