 ...
 */
function amb_site_display_block_options( $args = array() ) {
	...
	// Get the block ID.
	$block_id = amb_site_get_block_id( $args['block'] );
	...
	$has_show_overlay = amb_site_has_array_key( 'show_overlay', $background_options ) && true === $background_options['show_overlay'] ? ' has-overlay' : '';
	...
	if ( $args['background_type'] ) {
		...
		if ( 'video' === $args['background_type'] ) {
			...
			$background_alt = $background_title ? sprintf( esc_attr( 'Video Background of %s', 'advanced-maps-block' ), esc_attr( $background_options['background_video_title'] ) ) : __( 'Video Background', 'advanced-maps-block' );
			...
			?>
				...
				<button class="video-toggle"><span class="screen-reader-text"><?php esc_html_e( 'Toggle video playback', 'advanced-maps-block' ); ?></span></button>
			<?php
 ...
 * @return bool
 */
function amb_site_has_block_expired( $args = array() ) {
 ...
 */
function amb_site_acf_flexible_content_layout_title( $block_title, $field, $layout, $i ) {
	...
	if ( 'none' !== $background ) {
		...
		$type = amb_site_return_flexible_content_layout_value( $background_type );
		...
		if ( 'video' === $background_type ) {
			$block_heading .= '<div style="font-size: 30px; height: 26px; width: 30px;" class="dashicons dashicons-format-video acf-flexible-title-image"><span class="screen-reader-text">' . esc_html__( 'Video', 'advanced-maps-block' ) . '</span></div>';
		}
	}
	...
	if ( amb_site_has_block_expired(
		...
		$expired .= '<span style="color: red;">&nbsp;(' . esc_html__( 'expired', 'advanced-maps-block' ) . ')</span>';
	}
...
add_filter( 'acf/fields/flexible_content/layout_title/name=content_blocks', 'amb_site_acf_flexible_content_layout_title', 10, 4 );
 ...
 */
function amb_site_return_flexible_content_layout_value( $type ) {
...
if ( function_exists( 'amb_site_acf_flexible_content_layout_title' ) ) {

	/**
	 * Set Admin Styles for Flexible Content Layout Image/Title in amb_site_acf_flexible_content_layout_title().
	 *
	 * @author WDS
	 */
	function amb_site_flexible_content_layout_title_acf_admin_head() {
	?>
	<style type="text/css">
	...
	add_action( 'acf/input/admin_head', 'amb_site_flexible_content_layout_title_acf_admin_head' );
}

/**
 * Load colors dynamically into select field from amb_site_get_theme_colors()
 *
 * @author WDS
 ...
 */
function amb_site_acf_load_color_picker_field_choices( $field ) {

	// Reset choices.
	$field['choices'] = array();

	// Grab our colors array.
	$colors = amb_site_get_theme_colors();
...
add_filter( 'acf/load_field/name=color_picker', 'amb_site_acf_load_color_picker_field_choices' );
 ...
 */
function amb_site_get_theme_colors() {
	return array(
		esc_html__( 'Alto', 'advanced-maps-block' )           => '#ddd',
		esc_html__( 'Black', 'advanced-maps-block' )          => '#000',
		esc_html__( 'Blue', 'advanced-maps-block' )           => '#21759b',
		esc_html__( 'Cod Gray', 'advanced-maps-block' )       => '#111',
		esc_html__( 'Dove Gray', 'advanced-maps-block' )      => '#666',
		esc_html__( 'Gallery', 'advanced-maps-block' )        => '#eee',
		esc_html__( 'Gray', 'advanced-maps-block' )           => '#808080',
		esc_html__( 'Gray Alt', 'advanced-maps-block' )       => '#929292',
		esc_html__( 'Light Yellow', 'advanced-maps-block' )   => '#fff9c0',
		esc_html__( 'Mineshaft', 'advanced-maps-block' )      => '#333',
		esc_html__( 'Silver', 'advanced-maps-block' )         => '#ccc',
		esc_html__( 'Silver Chalice', 'advanced-maps-block' ) => '#aaa',
		esc_html__( 'White', 'advanced-maps-block' )          => '#fff',
		esc_html__( 'Whitesmoke', 'advanced-maps-block' )     => '#f1f1f1',
	);
}
 ...
 * @return void
 */
function amb_site_display_hero_heading( $block_title ) {
 ...
 * @since NEXT
 */
function amb_site_display_link( $args = array() ) {
	echo amb_site_get_link( $args ); // WPCS: XSS Ok.
}
 ...
 */
function amb_site_get_link( $args = array() ) {
	...
	$title = amb_site_has_array_key( 'title', $button_array ) ? $button_array['title'] : esc_html__( 'Read More', 'advanced-maps-block' );

	// Get url.
	$url = amb_site_has_array_key( 'url', $button_array ) ? $button_array['url'] : '';

	// Get target, else default internal.
	$target = amb_site_has_array_key( 'target', $button_array ) ? $button_array['target'] : '_self';
	?>
