 ...
 * @return array
 */
function amb_site_body_classes( $classes ) {
...
add_filter( 'body_class', 'amb_site_body_classes' );

/**
 * Flush out the transients used in amb_site_categorized_blog.
 *
 * @author WDS
 * @return string
 */
function amb_site_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( 'amb_site_categories' );
}
add_action( 'delete_category', 'amb_site_category_transient_flusher' );
add_action( 'save_post', 'amb_site_category_transient_flusher' );
 ...
 * @return string
 */
function amb_site_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'advanced-maps-block' ) . '...</a>';
}
add_filter( 'the_content_more_link', 'amb_site_content_more_link' );
 ...
 * @return string
 */
function amb_site_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'advanced-maps-block' ) );
}
add_filter( 'excerpt_more', 'amb_site_excerpt_more' );
 ...
 * @return array
 */
function amb_site_custom_mime_types( $mimes ) {
	$mimes['svg']  = 'image/svg+xml';
	$mimes['svgz'] = 'image/svg+xml';
	return $mimes;
}
add_filter( 'upload_mimes', 'amb_site_custom_mime_types' );
...
add_shortcode( 'svg', 'amb_site_display_svg' );
 ...
 * @return string
 */
function amb_site_display_customizer_header_scripts() {

	// Check for header scripts.
	$scripts = get_theme_mod( 'amb_site_header_scripts' );
	...
	echo amb_site_get_the_content( $scripts ); // WPCS XSS OK.
}
add_action( 'wp_head', 'amb_site_display_customizer_header_scripts', 999 );
 ...
 * @return string
 */
function amb_site_display_customizer_footer_scripts() {

	// Check for footer scripts.
	$scripts = get_theme_mod( 'amb_site_footer_scripts' );
...
add_action( 'wp_footer', 'amb_site_display_customizer_footer_scripts', 999 );
...
function amb_site_add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'advanced-maps-block' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'advanced-maps-block' ) : esc_html__( 'tagged with', 'advanced-maps-block' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = $card_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'advanced-maps-block' ), $specify, $term_name );
	}
	...
	if ( is_search() ) {
		...
		$card_long_description = $card_description = sprintf( esc_html__( 'Search results for %s.', 'advanced-maps-block' ), $search_term );
	}
...
add_action( 'wp_head', 'amb_site_add_og_tags' );
 ...
 * @author Corey Collins
 */
function amb_site_remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', 'amb_site_remove_archive_title_prefix' );
 ...
 * @author Corey Collins
 */
function amb_site_remove_empty_p_tags_from_content( $content ) {
...
add_filter( 'the_content', 'amb_site_remove_empty_p_tags_from_content' );
