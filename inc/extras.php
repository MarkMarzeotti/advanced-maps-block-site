 ...
 */
function amb_site_categorized_blog() {

	$category_count = get_transient( 'amb_site_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'amb_site_categories', $category_count );
	}
 ...
 */
function amb_site_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function amb_site_get_placeholder_unsplash( $args = array() ) {
	...
	if ( ! is_numeric( $args['width'] ) ) {
		throw new Exception( esc_html__( 'Width must be an integer', 'advanced-maps-block' ) );
	}

	if ( ! is_numeric( $args['height'] ) ) {
		throw new Exception( esc_html__( 'Height must be an integer', 'advanced-maps-block' ) );
	}
	...
	$alt = apply_filters( 'amb_site_placeholder_image_alt', esc_html__( 'WebDevStudios Placeholder Image', 'advanced-maps-block' ) );
 ...
 * @return string
 */
function amb_site_copyright_year( $atts ) {
...
add_shortcode( 'amb_site_copyright_year', 'amb_site_copyright_year', 15 );
...
if ( defined( 'WPSEO_VERSION' ) ) {
	 ...
	 * @since  NEXT
	 */
	function amb_site_move_yoast_to_bottom() {
		return 'low';
	}
	add_filter( 'wpseo_metabox_prio', 'amb_site_move_yoast_to_bottom' );
}
 ...
 * @author jomurgel
 */
function amb_site_get_the_content( $content ) {
...
add_filter( 'the_content', 'amb_site_get_the_content', 20 );
 ...
 */
function amb_site_get_theme_colors_gutenberg() {
