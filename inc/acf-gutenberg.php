...
function amb_site_acf_init() {
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-accordion',
			'title'           => esc_html__( 'Accordion', 'advanced-maps-block' ),
			'description'     => esc_html__( 'A custom set of collapsable accordion items.', 'advanced-maps-block' ),
			'render_callback' => 'amb_site_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'keywords'        => array( 'accordion', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'amb_site_acf_enqueue_accordion_scripts',
			'align'           => 'wide',
			'supports'        => $supports,
		)
	);
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-carousel',
			'title'           => esc_html__( 'Carousel', 'advanced-maps-block' ),
			'description'     => esc_html__( 'A carousel with a call to action for each slide.', 'advanced-maps-block' ),
			'render_callback' => 'amb_site_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'slides',
			'keywords'        => array( 'carousel', 'slider', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'amb_site_acf_enqueue_carousel_scripts',
			'align'           => 'wide',
			'supports'        => $supports,
		)
	);
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-cta',
			'title'           => esc_html__( 'Call To Action', 'advanced-maps-block' ),
			'description'     => esc_html__( 'A call to action block.', 'advanced-maps-block' ),
			'render_callback' => 'amb_site_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'megaphone',
			'keywords'        => array( 'call to action', 'cta', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'amb_site_acf_enqueue_backend_block_styles',
			'align'           => 'wide',
			'supports'        => $supports,
		)
	);
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Fifty/Fifty Block', 'advanced-maps-block' ),
			'description'     => esc_html__( 'A split-content block with text and/or media.', 'advanced-maps-block' ),
			'render_callback' => 'amb_site_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'editor-table',
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-hero',
			'title'           => esc_html__( 'Hero Block', 'advanced-maps-block' ),
			'description'     => esc_html__( 'A hero with an optional call to action.', 'advanced-maps-block' ),
			'render_callback' => 'amb_site_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'slides',
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Recent Posts Block', 'advanced-maps-block' ),
			'description'     => esc_html__( 'A set of recent posts displayed by category and/or tag.', 'advanced-maps-block' ),
			'render_callback' => 'amb_site_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'admin-page',
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Related Posts Block', 'advanced-maps-block' ),
			'description'     => esc_html__( 'A set of manually selected posts.', 'advanced-maps-block' ),
			'render_callback' => 'amb_site_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'admin-page',
...
add_action( 'acf/init', 'amb_site_acf_init' );
 ...
 * @author Corey Collins
 */
function amb_site_acf_block_registration_callback( $block ) {
	...
	if ( ! is_admin() && amb_site_has_block_expired(
		array(
	...
	amb_site_display_expired_block_message();
 ...
 * @author Corey Collins
 */
function amb_site_add_block_categories( $categories, $post ) {
	...
	return array_merge(
		...
		array(
			array(
				'slug'  => 'wds-blocks',
				'title' => esc_html__( 'WDS Blocks', 'advanced-maps-block' ),
			),
		)
	);
}
add_filter( 'block_categories', 'amb_site_add_block_categories', 10, 2 );
 ...
 * @author Corey Collins
 */
function amb_site_acf_enqueue_backend_block_styles() {
 ...
 * @author Corey Collins
 */
function amb_site_acf_enqueue_carousel_scripts() {

	if ( ! is_admin() ) {
		return;
	}

	amb_site_acf_enqueue_backend_block_styles();
 ...
 * @author Corey Collins
 */
function amb_site_acf_enqueue_accordion_scripts() {
 ...
 * @author Corey Collins
 */
function amb_site_get_block_alignment( $block ) {
 ...
 * @author Corey Collins
 */
function amb_site_get_block_classes( $block ) {
	...
	$classes  = '';
	$classes  = amb_site_get_block_expired_class();
 ...
 * @author Corey Collins
 */
function amb_site_get_block_expired_class() {
 ...
 * @author Corey Collins
 */
function amb_site_display_expired_block_message() {

	if ( ! amb_site_get_block_expired_class() ) {
		return;
	}

	?>
	<div class="block-expired-message">
		<span class="block-expired-text"><?php esc_html_e( 'Your block has expired. Please change or remove the Start and End dates under Other Options to display your block on the frontend.', 'advanced-maps-block' ); ?></span>
	</div>
	<?php
}
 ...
 * @author Corey Collins
 */
function amb_site_get_block_id( $block ) {
 ...
 * @author Corey Collins
 */
function amb_site_acf_gutenberg_display_admin_default_carousel( $block ) {
	...
	for ( $slides = 0; $slides < 2; $slides++ ) :
		...
		<section class-"slide">
			<div class="slide-content container">
				<h2 class="slide-title"><?php esc_html_e( 'Slide Title', 'advanced-maps-block' ); ?></h2>
				<p class="slide-description"><?php esc_html_e( 'Slide Content', 'advanced-maps-block' ); ?></p>
			</div>
		</section>
	<?php
	endfor;
